<template>
  <el-dialog v-if="dialogVisible" width="80%" height="80%" :title="title" :visible.sync="dialogVisible">
    <el-main>
      <table class="layui-table el-table__body">
        <tbody>
          <tr>
            <td class="right">请选择要添加的部门：</td>
            <td>
              <el-select v-model="depId" @change="changeDept">
                <el-option v-for="item in deptData" :label="item.depName" :value="item.depId" :key="item.depId"></el-option>
              </el-select>
            </td>
          </tr>
          <tr>
            <td class="right">该部门下已选人员：</td>
            <td>
    ...
    <el-row :gutter="16" class="m-3">
      <el-col :span="6" class="border p-3">
        <el-tree
          ref="tree"
          :data="depData"
          node-key="id"
      ...
      <el-col :span="17" class="border p-3 ml-2">
        ...
        <el-row style="margin-top: 10px">
          <el-col :span="24">
            ...
            <pagination :total="pageTotal" :current-page="query.pageIndex" :pageSize="query.pageSize" @pageChange="handlePageChange" />
          </el-col>
        </el-row>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    fetchUser() {
      this.loading = true;
      this.$api.IdmUser.GetUserList(this.query.userName, this.query.departmentCode, this.query.skipCount, this.query.pageSize).then(
        (res) => {
          this.loading = false;
    ...
    getSelectUser() {
      ...
      let arr = this.deptData.find((item) => {
        return item.depId == this.depId; //返回满足条件的第一个值
      });
      if (arr) {
        data.depName = arr.depName;
      }

      this.$api.Evaluate.GetEvalPersion(data).then((res) => {
        this.selectUser = res;
      });
    },
    ...
    onAdd(user) {
      if (this.depId) {
        ...
        data.depName = arr.depName;
        data.depQz = arr.depQz;

        this.$api.Evaluate.InsertEvalPersion(data).then((res) => {
          console.log("res=", res);
          if (res.success) {
    ...
    clearUser(index) {
      ...
      this.$api.Evaluate.DelEvalPerson({ id: u.id }).then((res) => {
        console.log("res=", res);
        if (res.success) {
