  ...
  <div>
    <table class="layui-table el-table__body">
      <tbody>
        ...
        <tr>
          <td class="center" rowspan="3">团队考核</td>
          <td class="center">员工资质</td>
          <td class="left" colspan="6">
            1、本月不符合任职要求员工<el-input class="line" style="width: 100px" v-model.number="dataval[28]" />人次，每人次扣除200元；
          </td>
          <td class="center"><el-input class="line" style="width: 100px" v-model.number="dataval[29]" /></td>
          <td class="center" rowspan="3">依据合同条款执行</td>
        </tr>
        <tr>
          <td class="center" rowspan="2">团队 稳定性</td>
          <td class="left" colspan="6">1、乙方【是/否】连续两个月员工月度更换率超过50%，每出现1起扣除乙方2000元；</td>
          <td class="center"><el-input class="line" style="width: 100px" v-model.number="dataval[30]" /></td>
        </tr>
        <tr>
          <td class="left" colspan="6">2、乙方【是/否】出现20%以上员工罢工、不在岗等未履行工作职责的，每出现1起扣除乙方服务费2000元。</td>
          <td class="center"><el-input class="line" style="width: 100px" v-model.number="dataval[31]" /></td>
        </tr>
        <tr>
          <td class="center" rowspan="7">服务过程考核</td>
          <td class="center">保洁基础业务品质</td>
          <td class="left" colspan="6">
            本月度保安服务月度专项检查得分<el-input class="line" style="width: 100px" v-model.number="dataval[32]" />分，<br />
            1：检查结果＞85分（含），不扣减；，<br />
        ...
        <tr>
          ...
          <td class="center"><el-input class="line" style="width: 100px" v-model.number="dataval[37]" /></td>
          <td class="center"><el-input class="line" v-model="dataval[38]" /></td>
        </tr>
        <tr>
          <td class="left" colspan="6">
            3、乙方员工盗窃、私拿甲方业主或员工物品的；乙方员工恐吓、威胁甲方业主或员工，与甲方业主或员工发生肢体冲突的，扣除服务费5000元，从第2起按1.5倍增扣；
          </td>
          <td class="center"><el-input class="line" style="width: 100px" v-model.number="dataval[39]" /></td>
          <td class="center"><el-input class="line" v-model="dataval[40]" /></td>
        </tr>
        <tr>
          <td class="left" colspan="6">4、乙方员工殴打甲方业主或员工的，致使造成轻微伤及以上的，扣除月度服务费的50%但不低于5万元/次。</td>
          <td class="center"><el-input class="line" style="width: 100px" v-model.number="dataval[41]" /></td>
          <td class="center"><el-input class="line" v-model="dataval[42]" /></td>
        </tr>
        <tr>
          <td class="left" colspan="6">5、乙方员工酗酒上岗、睡岗、擅自脱岗/离岗，每出现一起，扣除乙方服务费人民币2000元；</td>
          <td class="center"><el-input class="line" style="width: 100px" v-model.number="dataval[43]" /></td>
          <td class="center"><el-input class="line" v-model="dataval[44]" /></td>
        </tr>
        <tr>
          <td class="left" colspan="6">6、乙方员工在公共场所睡觉，聚岗聊天未造其他后果的。每出现一起，扣除乙方服务费人民币500元。</td>
          <td class="center"><el-input class="line" style="width: 100px" v-model.number="dataval[45]" /></td>
          <td class="center"><el-input class="line" v-model="dataval[46]" /></td>
        </tr>
        <tr>
          <td class="center" colspan="2">违约扣款合计</td>
          <td class="left" colspan="8">总计金额：<el-input class="line" style="width: 150px" v-model.number="dataval[47]" />元</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td class="left" colspan="8">
            (合同约定月度应支付服务费)<el-input class="line" style="width: 150px" v-model.number="dataval[48]" />
          </td>
        </tr>
        <tr>
          <td class="center" colspan="2">本月供方评价 违约扣款金额</td>
          <td class="left" colspan="8">
            （执行供方总体评价金额）<el-input class="line" style="width: 150px" v-model.number="dataval[49]" />
          </td>
        </tr>
        <tr>
          <td class="center" colspan="2">本月合同实付 含税金额</td>
          <td class="left" colspan="8">
            （月服务费标准减去供方总体评价金额）<el-input class="line" style="width: 150px" v-model.number="dataval[50]" />
          </td>
        </tr>
        <tr>
          ...
          <td class="left" colspan="8">
            (（本月合同实付含税金额/1.06）
            <el-input class="line" style="width: 150px" v-model.number="dataval[51]" />
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td class="left" colspan="10">
            备注： <br />考核以《保洁服务外包合同》、《服务质量考核细则》及其他相关附件为依据。<br />
      ...
      this.$http.get("/StartEvaluate/GetMonthEvalContent?groupId=" + this.groupId).then((res) => {
        if (res.status == 200) {
          ...
          if (res.data.result.length > 0) {
            let start =
              res.data.result[52].indexOf("T") > -1
                ? res.data.result[52].substring(0, res.data.result[52].indexOf("T")).split("-")
                : res.data.result[52].split("-");
            let end =
              res.data.result[53].indexOf("T") > -1
                ? res.data.result[53].substring(0, res.data.result[53].indexOf("T")).split("-")
                : res.data.result[53].split("-");
            this.value1 = [
    ...
    submit() {
      ...
      if (isOk) {
        this.$confirm("确定提交吗?", "提示", {
          ...
          this.dataval[52] = this.value1[0];
          this.dataval[53] = this.value1[1];
          let obj = {
            id: this.groupId,
            startTime: this.value1[0],
            endTime: this.value1[1],
            defaultdeductionTotal: this.dataval[47], //违约扣款合计
            payableTaxcontainingAmount: this.dataval[48], //本月合同应付含税金额
            defaultDeduction: this.dataval[49], //本月合同应付含税金额
            actualTaxcontainingAmount: this.dataval[50], //本月合同实付含税金额
            exclusiveOfTax: this.dataval[51], //本月合同实付不含税金额
