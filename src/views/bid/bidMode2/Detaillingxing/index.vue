  ...
  <el-main>
    ...
    <el-row>
      <div style="float:right;height:50px">
        <el-button type="primary" v-if="showedit" class="buttoncss" size="small" @click="onSubmit('form1')">保存</el-button>
      </div>
    </el-row>
    <el-form :model="form1" :rules="rules" ref="form1" size="small">
      <div class="fromborder">
        <el-row :gutter="24">
          <el-col :span="8">
            <el-form-item label="竞争性判断原因" prop="reason" v-if="showedit">
            ...
            <el-form-item label="竞争性判断原因" prop="reason" v-else>
              <el-select disabled style="width: 100%" v-model="form1.reason" placeholder="请选择">
                <el-option v-for="item in reasons" :key="item.value" :label="item.text" :value="item.value"> </el-option>
              </el-select>
            </el-form-item>
          </el-col>
    ...
    <el-button type="primary" v-if="showedit" style="float:right;margin-bottom:10px" size="small" @click="add">添加供方</el-button>
    <el-button type="primary" v-if="showedit" style="float:right;margin-bottom:10px;margin-right:10px" size="small" @click="savePerson"
      >保存联系人</el-button
    >
    <el-table empty-text="无符合条件的记录" stripe border element-loading-text="请稍候,数据正在加载中..." :data="tableData">
      ...
      <el-table-column label="供方联系人" width="150" align="center">
        <el-table-column label="姓名" width="150" align="center" prop="personName">
          <template slot-scope="scope">
            <el-input v-model="scope.row.personName" v-if="showedit"></el-input>
            <el-input disabled v-model="scope.row.personName" v-else></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="职务" width="150" align="center" prop="workAs">
          <template slot-scope="scope">
            <el-input v-if="showedit" placeholder="请输入职务" v-model="scope.row.workAs"></el-input>
            <el-input v-else disabled placeholder="请输入职务" v-model="scope.row.workAs"></el-input>
          </template>
        </el-table-column>
      </el-table-column>
      <el-table-column label="操作" width="80" align="center" v-if="showedit">
        <template slot-scope="scope">
          <i class="el-icon-delete" title="删除" @click="onDeleteData(scope.row)"></i>
        </template>
      </el-table-column>
    </el-table>
