...
export default {
  ...
  methods: {
    check() {
      this.$api.BidModeMasterOther.CheckSubmitLinXing(this.baseId).then((res) => {
        this.status = res.success;
        this.checkList = res.checkItemList;
      });
    },
    onSubmit() {
      this.$api.BidModeMasterOther.SubmitBidModeLinXing(this.baseId).then((res) => {
        if (res.success) {
