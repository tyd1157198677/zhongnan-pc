    ...
    <el-row id="page_main">
      ...
      <el-col :span="20" style="margin-bottom: 50px">
        ...
        <div class="alert alert-primary" role="alert" style="margin: 10px">
          ...
          <p class="h6">3、添加集采供应商</p>
          <p class="h6">4、添加采购小组成员</p>
        </div>
...
export default {
  ...
  methods: {
    check() {
      this.$api.BidModeMasterOther.CheckSubmitJiCai(this.baseId).then((res) => {
        this.status = res.success;
        this.checkList = res.checkItemList;
      });
    },
    onSubmit() {
      this.$api.BidModeMasterOther.SubmitBidModeJiCai(this.baseId).then((res) => {
        if (res.success) {
