    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="内部文件答疑" name="内部文件答疑">
        <div class="btns" style="margin-bottom: 20px">
          ...
          <div v-if="isBidUser">
            <el-button style="float: right; margin-bottom: 10px" type="danger" v-if="showUploadButton" size="mini" @click="upLoadDaYiFile"
              >上传答疑文件</el-button
            >
          </div>
        </div>
          ...
          <el-collapse-item name="1" v-for="(item, index) in daYiFileList" :key="index" style="margin-top: 20px">
            <template slot="title">
              <span class="infonametits"
                ><strong>{{ item.content }}</strong></span
              >
            </template>
        ...
        <el-collapse v-model="activeNames">
          <el-collapse-item name="1" v-for="(item, index) in dataList2" :key="index" style="margin-top: 20px">
            ...
            <div style="margin-top: 50px" v-if="tableif3">
              ...
              <div class="text item">
                <el-table empty-text="暂无数据" stripe border element-loading-text="请稍候,数据正在加载中..." :data="tableshenpiwaibu">
                  <el-table-column type="index" width="60" label="序号" align="center"> </el-table-column>
                  <el-table-column prop="activityNameEnum" width="200" label="步骤" align="center"> </el-table-column>
                  <el-table-column prop="userName" label="经办人" width="200" align="center"> </el-table-column>
      ...
      <el-form label-width="80px" size="small" class="m-3">
        ...
        <el-row>
          <el-col :span="24" class="right" style="margin-bottom: 20px">
            <el-button type="primary" size="small" @click="submiteva1()">保存</el-button>
...
export default {
  components: { publicUpload: () => import("@/components/publicUpload.vue") },
  props: ["id"],
  data() {
    return {
      value: "",
      ...
      rulesrizhi: {
        ...
        // ideaRemarkneibu: [{ required: true, message: "审批意见不能为空", trigger: "blur" }],
      },
      dataList3: [],
      tableListwaibu: [],
  ...
  methods: {
    ...
    omsubmit() {
      this.$confirm("确认要通过审批吗?", "提示", {
        ...
        if (!this.ideaRemark == "") {
          let obj = {
            bidModeId: this.id,
            masterId: this.selectExplainId1,
            remark: this.ideaRemarkneibu,
            isOk: 0,
          };
    ...
    GetQustionList() {
      this.$http.get("/LiandoBidMaster/GetQustionList?bidModeId=" + this.id).then((res) => {
        ...
        this.selectExplainId1 = res.data.dataList.length > 0 ? res.data.dataList[0].id : "";
        if (this.daYiFileList.length > 0) {
          this.GetExplainDetailList3211();
          this.GetQustionFileList();
        }
      });
    },
    ...
    GetNewLogList_SupQus(row) {
      this.$http.get("/LiandoBidSupQs/GetAnswerMaster001", { params: { bidModeId: this.id } }).then((res) => {
        // if (res.data.logs.length > 0) {
        this.tableshenpiwaibu = res.data.logData.logs;
    ...
    submiteva1() {
      if (this.message == "") {
        ...
        this.$http.post("/LiandoBidMaster/ReturnToDoToDesign", { bidModeId: this.id, message: this.message }).then((res) => {
          if (res.data.success) {
            this.$message.success(res.data.message);
            this.GetGetDesignCode();
          } else {
