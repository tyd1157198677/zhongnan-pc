  ...
  <div class="">
    ...
    <el-row>
      <div style="float:right;height:50px">
        <el-button type="primary" class="buttoncss" size="small" v-if="showedit" @click="onSubmit('from1')">保存</el-button>
      </div>
    </el-row>
    <el-form :model="form1" :rules="rules" ref="form1" size="small">
      <div class="fromborder">
        <el-row :gutter="24" style="margin-right: 0px;margin-left:0px">
          <el-col :span="8">
            <el-form-item label="中标单位" prop="supplierName">
              <el-select style="width: 100%" v-if="showedit" v-model="form1.supplierName" placeholder="请选择">
                <el-option v-for="item in reasons" :key="item.value" :label="item.text" :value="item.value"> </el-option>
              </el-select>
              <el-select style="width: 100%" v-else disabled v-model="form1.supplierName" placeholder="请选择">
                <el-option v-for="item in reasons" :key="item.value" :label="item.text" :value="item.value"> </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="中标金额（元）" v-if="showedit" prop="finalWinPrice">
              <el-input style="width:100%" v-model="form1.finalWinPrice" placeholder="请输入"> </el-input>
            </el-form-item>
            <el-form-item label="中标金额（元）" v-else prop="finalWinPrice">
              <el-input style="width:100%" disabled v-model="form1.finalWinPrice" placeholder="请输入"> </el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="16">
            <el-form-item label="相关说明" v-if="showedit" prop="winDesc">
              <el-input style="width:100%" type="textarea" v-model="form1.winDesc"></el-input>
            </el-form-item>
            <el-form-item label="相关说明" v-else prop="winDesc">
              <el-input style="width:100%" disabled type="textarea" v-model="form1.winDesc"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8"></el-col>
        </el-row>
      ...
      <el-row>
        ...
        <el-table empty-text="无符合条件的记录" stripe border element-loading-text="请稍候,数据正在加载中..." :data="fileslist">
          ...
          <el-table-column label="操作" width="80" align="center">
            <template slot-scope="scope">
              <i class="el-icon-delete" title="删除" v-if="showedit" @click="deleteRow(scope.row)"></i>
        ...
        <el-form-item label="选择文件">
          ...
          >
            ...
            <div slot="tip" class="el-upload__tip">
              允许文件类型：{{ getOneFileTemp.fileExt }} 最大：{{ getOneFileTemp.maxFileSize }} 兆
            </div>
          </el-upload>
...
export default {
  ...
  mounted() {
    ...
    this.CheckPurchaseWinLxEdit3001();
    this.getList2();
  ...
  methods: {
    ...
    CheckPurchaseWinLxEdit3001() {
      this.$http.get("/PurchaseModeLx/CheckPurchaseWinLxEdit3001", { params: { bidModeId: this.bidModeId } }).then(res => {
        if (res.data.success) {
          this.showedit = true;
        } else {
          this.showedit = false;
    ...
    //   const fileSuffix = file.name.substring(file.name.lastIndexOf("."));
    //   const whiteList = this.FileNamess.fileExt.split("|");
    //   console.log(file);
    //   // console.log(fileSuffix);
    //   // console.log(whiteList);
    //   if (whiteList.indexOf(fileSuffix) === -1) {
    ...
    //     return false;
    //   }
    //   // const isLt = file.size / 1024 / 1024 < this.uploadData.maxFileSize;
    //   // if (!isLt) {
    //   //   this.$message.error("上传文件大小不能超过" + this.uploadData.maxFileSize + "兆");
    //   //   return false;
    //   // }
    // },
    onSubmit() {
