<template>
  <el-container class="detail">
    <el-main>
      ...
      <ding-biaoZw v-if="text == '招标方案'" :id="id"></ding-biaoZw>
    </el-main>
  </el-container>
</template>
...
export default {
  components: {
    ...
    dingBiaoZw: () => import("./components/dingBiaoLxan"),
    ...
    this.$http.get("/UserSession/GetUserProcessAuth", { params: { bidModeId: this.id } }).then((res) => {
      ...
      if (!tab) {
        if (indexactive == null || indexactive == -1) {
          if (arr.indexOf("招标方案") > -1) {
            sessionStorage.setItem("indexactive", 0);
            this.indexactive = 0;
            this.tabto == "招标方案";
            this.text = "招标方案";
            this.titleList[indexactive].text = "招标文件";
          } else if (arr.indexOf("招标文件") > -1) {
