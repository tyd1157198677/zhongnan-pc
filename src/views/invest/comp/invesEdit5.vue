      ...
      >
        ...
        <el-table-column align="left">
          <template slot-scope="scope">
              ...
              ></el-input>
              <el-radio-group v-if="scope.row.itemType == '单选'" v-model="scope.row.formValue" style="width:100%">
                <el-radio v-for="item in scope.row.selectorNameValue" :label="item.selectorName" :key="item.selectorName">{{
                  item.selectorName
                  }}</el-radio>
              </el-radio-group>
              <el-checkbox-group v-model="scope.row.formValue" v-if="scope.row.itemType == '多选'" style="width:100%">
                <el-checkbox v-for="item in scope.row.selectorNameValue" :label="item.selectorName" :key="item.selectorName">
                  {{ item.selectorName }}
                </el-checkbox>
              </el-checkbox-group>
              <el-select v-if="scope.row.itemType == '下拉'" v-model="scope.row.formValue" placeholder="请选择" style="width:100%">
                <el-option
...
export default {
  components: {
    ...
    methods: {
      getList(){
        this.$api.invest.GetModuleDynamicForm4({masterId:this.$route.query.id}).then(res => {
          res.forEach(item => {
