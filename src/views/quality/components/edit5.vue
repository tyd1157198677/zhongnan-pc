...
<template>
  <div class="">
    <el-breadcrumb class="table-top-title" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item><b>5.总包在施项目业绩汇总表（含实际承包人）</b></el-breadcrumb-item>
    </el-breadcrumb>
    <table class="layui-table el-table__body">
      <tbody>
        <tr>
          ...
          <td>
            <span v-if="type == 2">{{ base.projectCount }}</span>
            <el-input
              class="line"
              style="width:85%"
              type="number"
          ...
          <td>
            <span v-if="type == 2">{{ base.buildArea }}</span>
            <el-input
              class="line"
              style="width:85%"
              type="number"
              v-else
              v-model="base.buildArea"
              :min="0"
              placeholder="请填写建筑面积"
            ></el-input
            >㎡
          ...
          <td>
            <span v-if="type == 2">{{ base.cooperateRatio }}</span>
            <el-input
              class="line"
              style="width:85%"
              type="number"
          ...
          <td>
            <span v-if="type == 2">{{ base.typeRatio }}%</span>
            <el-input
              class="line"
              style="width:85%"
              type="number"
        ...
        <tr>
          <td colspan="8">
            <div class="between">
              <span>备注：</span>
              <span>
                <el-button v-if="type == 1" type="primary" size="mini" @click="save">暂存信息</el-button>
              </span>
            </div>
    ...
    <el-table :data="companyYeJi" stripe border v-loading="loading">
      ...
      <el-table-column label="地产简称" min-width="160" align="center" prop="abbr"> </el-table-column>
      <el-table-column align="center" min-width="160" label="项目甲方" prop="partner" show-overflow-tooltip>
        <template slot-scope="scope">
          <div class="sp">{{ scope.row.partner }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="项目名称" prop="projectName"> </el-table-column>
      <el-table-column align="center" label="项目地点" prop="projectAddress"> </el-table-column>
      <el-table-column align="center" label="业态" prop="formatAndMes" width="120"> </el-table-column>
      <el-table-column align="center" label="建面(平米)" prop="buildArea" width="100"> </el-table-column>
      <el-table-column align="center" label="合同金额(万元)" prop="contractAmount" width="120"> </el-table-column>
      <el-table-column align="center" label="开，竣工时间" prop="projectStartTime" width="120">
        <template slot-scope="scope">
          <div>{{ scope.row.projectStartTime }}</div>
          <div>{{ scope.row.projectEndTime }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="当前项目进展" prop="proProcess" width="120"> </el-table-column>
      <el-table-column align="center" label="项目承接模式" prop="comMainType" width="120"> </el-table-column>
      <el-table-column align="center" label="实际承包人" prop="realContractor" width="100"> </el-table-column>
      <el-table-column align="center" label="项目经理" prop="manager" width="100"> </el-table-column>
      <el-table-column align="center" label="甲方联系人及电话" width="115">
        <template slot-scope="scope">
          <div>{{ scope.row.aLiaisonMan }}</div>
          <div>{{ scope.row.aPhone }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="混凝土联系人及电话" width="115">
        <template slot-scope="scope">
          <div>{{ scope.row.bLiaisonMan }}</div>
          <div>{{ scope.row.bPhone }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="砌块联系人及电话" width="115">
        <template slot-scope="scope">
          <div>{{ scope.row.cLiaisonMan }}</div>
          <div>{{ scope.row.cPhone }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="木工联系人及电话" width="115">
        <template slot-scope="scope">
          <div>{{ scope.row.dLiaisonMan }}</div>
          <div>{{ scope.row.dPhone }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="瓦工联系人及电话" width="115">
        <template slot-scope="scope">
          <div>{{ scope.row.fLiaisonMan }}</div>
          <div>{{ scope.row.fPhone }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="钢筋工联系人及电话" width="115">
        <template slot-scope="scope">
          <div>{{ scope.row.eLiaisonMan }}</div>
          <div>{{ scope.row.ePhone }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="备注" prop="remark" width="100"></el-table-column>
      <el-table-column v-if="type == 1" label="操作" align="center" width="80">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getInfo(projectCount, buildArea, cooperateRatio, typeRatio) {
      this.base.projectCount = projectCount;
      this.base.buildArea = buildArea;
      this.base.cooperateRatio = cooperateRatio;
      this.base.typeRatio = typeRatio;
    },
    //获取总、分公司代表业绩（含实际承包人
