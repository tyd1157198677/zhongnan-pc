...
<template>
  <div class="">
    ...
    <div class="btns">
      ...
      <el-upload  v-if="isEdit == 1" style="margin:0 10px" class="upload" action="#" multiple :show-file-list="false" :http-request="uploadFile">
    ...
    <table class="layui-table el-table__body">
      <thead>
        <tr>
          <th v-for="(ths,thIndex) in tableHeader" :key="thIndex"  v-text="ths.Name" style="white-space: pre-line; text-align: center;   background-color: lightgray;
    color: #333333;"></th>
          <th v-if="isEdit == 1" style=" text-align: center;   background-color: lightgray;
    color: #333333;">操作</th>
        </tr>
      </thead>
      <tbody>
...
export default {
  ...
  methods: {
    ...
    async GetSupQuaFormList_PersionList() {
      this.$http.get("/SupQua/GetSupQuaFormList_PersionList",{
        params:
        { 
          masterId: this.$route.query.id,
          type: '含实际承包人'
    ...
    async GetSupQuaFormList_Persion() {
      this.$http.get("/SupQua/GetSupQuaFormList_Persion",{
        params:
        { 
          masterId: this.$route.query.id,
          type: '含实际承包人'
    ...
    // 导出模板
    downLoad() {
      window.open(`${process.env.VUE_APP_API_URL}/api/SupplierQua/GetExportTempletSupQuaFormList_Persion?masterId=${this.$route.query.id}` , "_blank");
    },
    ...
    save6() {
      ...
      let arr = st.map(item => {
        return {
          itemId: item.item.id,
          id: item.id,
          itemSelectorId: (item.itemType == "单选" || item.itemType == "多选" || item.itemType == "下拉" )? item.selectValue : ['00000000-0000-0000-0000-000000000000'],
      ...
      this.$http
        .post("/SupQua/SaveSupQuaFormList_Persion", obj)
        .then(res => {
          let data = res.data;
          if (data.success) {
