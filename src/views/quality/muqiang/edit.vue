<template>
  <div style="margin:0px auto 10px;margin-bottom: 65px;width:90%;">
    <div style="text-align:right;margin-right:13px">
      <span class="headername">幕墙专业-资格预审表</span>
    ...
    <el-row class="pt-3 pl-3 pr-3 mx-auto mh">
      ...
      <edit2 :message="messageTitle"></edit2>
    </el-row>
    <el-row class="pt-3 pl-3 pr-3 mx-auto mh">
      <div class="table-top-title" style="margin-bottom: 10px">生产加工厂情况</div>
      <edit3></edit3>
    </el-row>
    <el-row class="pt-3 pl-3 pr-3 mx-auto mh" v-for="(item, index) in formList" :key="index">
      ...
      <form-edit :dataArr="item.allModuleDynamicForm" :basetype="type"></form-edit>
    </el-row>

    <el-row class="pt-3 pl-3 pr-3 mx-auto mh">
      <div class="table-top-title" style="margin-bottom: 10px">拟派驻场项目负责人来公司前的履历</div>
      <lvli :type="type"></lvli>
    </el-row>
    <el-row class="pt-3 pl-3 pr-3 mx-auto mh">
      <div class="table-top-title" style="margin-bottom: 10px">拟派驻场项目负责人有代表性的工程业绩</div>
      <head-project-results :type="type"></head-project-results>
    </el-row>
    ...
    <el-row class="pt-3 pl-3 pr-3 mx-auto mh">
      ...
      <el-button type="primary" size="small" @click="tiJiao" style="float: right;margin-top: 10px;">提交资格预审信息</el-button>
    </el-row>
...
import headProjectResults from "./headProjectResults";
import lvli from "@/views/quality/components/lvli";
...
export default {
  ...
  data() {
    return {
      ...
      baseGuid: "", // 企业基本id
      messageTitle:
        "请填报【董事长】、【总公司总经理】、【对接分公司总经理】、【经营部负责人或对接人】、【工程部负责人】、【驻场项目负责人】相关信息",
      loading: false,
      formList: [],
    };
  },
  ...
  methods: {
      ...
      this.$http
        .get("/InvesLianDoFromNew/GetModuleDynamicFormAll", {
          params: {
            ...
            dynamicModule: "资格预审_幕墙",
          },
        })
        .then(res => {
          console.log("资格预审_幕墙", res);
    ...
    tiJiao() {
      this.$confirm("提交后不能修改,确认要提交吗?", "提示", {
        ...
        .then(() => {
          this.$http.post("/SupQua/SubmitQualityStatus", { id: this.baseGuid }).then(res => {
            if (res.data.success) {
        ...
        .catch(() => {
          this.$msg({ type: "info", message: "已取消提交" });
        });
    },
    quxiao() {
      this.$router.go(-1);
