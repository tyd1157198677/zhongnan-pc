    ...
    <el-row id="page_main">
      ...
      <div class="four">
        ...
        <div class="btn1" style="justify-content: flex-end;background:#fff;">
          <el-button type="primary" size="mini" v-if="type == 1" @click="add(4)">新增参与资格预审人员</el-button>
        </div>
          ...
          <el-table-column align="left">
            <template slot-scope="scope">
              <div v-if="type == 2">
                ...
                <el-button
                  @click="downLoad5(scope.row.fileUrl)"
            ...
            <template slot-scope="scope">
              ...
              <div v-else>
                <el-input
                  ...
                  :placeholder="scope.row.item.defaultValue"
                  v-if="
                    scope.row.item.itemType == '单行文本' || scope.row.item.itemType == '多行文本' || scope.row.item.itemType == '文本标题'
                  "
                ></el-input>
                <el-radio-group v-if="scope.row.item.itemType == '单选'" v-model="scope.row.textValue">
                  <el-radio v-for="item in scope.row.selectorList" :label="item.selectorId" :key="item.selectorId">{{
      ...
      <div class="ten" style="margin-bottom:20px">
        <el-breadcrumb class="table-top-title" separator-class="el-icon-arrow-right">
          <el-breadcrumb-item><b>9.在施项目业绩汇总表（用于后补使用）</b></el-breadcrumb-item>
        </el-breadcrumb>
        <div class="btns">
          <!-- <el-button v-if="type == 1" type="primary" size="mini" @click="add(8)">新增</el-button> -->
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="职务" prop="workAs">
              <el-select v-model="formSearch4.workAs" placeholder="请选择">
          ...
          <el-col :span="8">
            <el-form-item label="是否主要联系人">
              <el-select v-model="formSearch4.isHeadPerson" placeholder="请选择">
...
import BeiJing from "../invest/comp/beiJing.vue";
export default {
  components: {
  ...
  data() {
    return {
      ...
      fileType: [".pdf", ".word"],
      fileType5: [],
      condition: "允许文件类型：.pdf|.word 格式 最大：50 兆",
      condition5: "",
  ...
  methods: {
    ...
    save3() {
      this.takeOnType.forEach(item => {
        if (!item.ration) {
          this.$message.warning("选择该模式的时候占比为必填项");
          return false;
        } else {
    ...
    upDownFile(i) {
      if (i == 1) {
        this.fileType5 = [".png", ".jpg", ".pdf", ".xls", ".xlsx", ".rar", ".zip"];
    ...
    // downLoad5(file) {
    //   return `${process.env.VUE_APP_API_URL}/${file.fileUrl}`;
    // },
    upLoadBack(val) {
    ...
    save7() {
      ...
      let arr = st.map(item => {
        return {
          itemId: item.item.id,
          itemSelectorId:
            item.item.itemType == "单行文本" || item.item.itemType == "多行文本" || item.item.itemType == "文本标题"
              ? ["00000000-0000-0000-0000-000000000000"]
              : item.item.itemType == "多选"
              ? item.selectValue
              : [item.textValue],
          value:
            item.item.itemType == "单行文本" || item.item.itemType == "多行文本" || item.item.itemType == "文本标题" ? item.textValue : "",
        };
      });
      let obj = {
        masterId: this.id,
    ...
    save8() {
      this.$refs.saveSupForm8.validate(valid => {
        if (valid) {
          this.$api.quality.InsterManagerMes(this.formSearch8).then(res => {
            if (res.success) {
