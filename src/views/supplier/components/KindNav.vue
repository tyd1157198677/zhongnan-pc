  ...
  <div style="padding-right: 10px">
    <ul class="list-group" style="position: relative">
      ...
      <menu v-for="(item, index) in cates" :key="item.id" @mouseenter="onMouseEnter(item.id)" @mouseleave="onMouseLeave(item.id)">
        ...
        <div class="cat-subcategory" :id="`con-${item.id}`" style="margin-left: -10px">
          <div class="shadow" style="overflow: auto">
            <div class="entity-main">
              <ul class="shadow-left">
                <li v-for="second in item.children" :key="second.id">
                  <h3 class="mt-2 hand" @click="itemClick(`${second.cateName}`)">{{ second.cateName }}</h3>
                  <!-- <a class="hand" v-for="third in second.children" :key="third.id" @click="itemClick(`${third.cateName}`)">{{ -->
                  <!-- third.cateName
                  }}</a> -->
                </li>
              </ul>
