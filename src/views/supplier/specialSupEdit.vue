  ...
  <el-main>
    ...
    <el-card class="box-card" shadow="never">
      ...
      >
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="供方名称：" prop="comFullName">
              <el-input v-model.trim="formSearch.comFullName" maxlength="50" placeholder="供方名称"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="主营分类：" prop="supCateNames">
              <el-input v-model="formSearch.supCateNames" placeholder="请填写供方主营分类" @click.native="onOpenCategoryTree"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
          ...
          <el-col :span="8">
            <el-form-item label="注册机构：" prop="orgId">
              ...
              <el-input placeholder="请选择项目名称" style="width: 100%" v-model.trim="orgName" @focus="subJectAdd()"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <!-- <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="收款银行全称：" prop="bankName">
                <el-input v-model="formSearch.bankName" maxlength="50" placeholder="如：中国建设银行北京分行"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="收款银行账号：" prop="bankAccount">
                <el-input
                  v-model="formSearch.bankAccount"
                  placeholder="如:8888888888888"
                  maxlength="50"
            ...
            <el-col :span="8">
              <el-form-item label="行号：">
                <el-input
                  v-model="formSearch.openAccountNum"
                  placeholder="如:8888888888888"
                  onkeyup="this.value=this.value.replace(/[^\d.]/g,'');"
                ></el-input>
              </el-form-item>
    ...
    <bank-info v-if="baseGuid"></bank-info>
    <el-card class="box-card" shadow="never" style="margin: 10px 0" v-if="baseGuid">
      <div class="right">
      ...
      <el-form ref="userForm" :model="userForm" :rules="userRules" label-width="130px" size="small" class="form">
        ...
        <el-row>
          <el-col :span="24" class="right" style="margin-bottom: 20px">
            ...
            <el-button type="default" size="small" class="buttoncss" @click="dialogVisible = false" style="float: right; margin-right: 10px"
              >取消</el-button
            >
          </el-col>
        </el-row>
    ...
    <category-tree ref="categoryTree" :max="1" @select="onSelectedCategoryTree" />
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        citys: { required: true, message: "请选择业务城市", trigger: "blur" },
        ...
        bankName: { required: true, message: "请输入银行全称", trigger: "blur" },
        directCause: { required: true, message: "请选择供方类型", trigger: "change" },
  ...
  mounted() {
    ...
    if (this.$route.query.id && this.$route.query.id != null) {
      this.getOutList(this.$route.query.id);
    }
  ...
  methods: {
    ...
    getOutList(id) {
      ...
      this.$http.get("/SupBaseInfo/GetOneUserRegSupplier", { params: obj }).then((res) => {
        ...
        this.formSearch.supCateNames = res.data.supCateNames;
        this.id = res.data.id;
    ...
    submitForm() {
      ...
      if (arr.length > 10) {
        ...
        this.$refs["saveSupForm"].clearValidate(["citys"]); //取消该项校验
    ...
    onSubmitsave() {
      // if (this.formSearch.directCause == "" || this.formSearch.directCause == null) {
      //   this.$message({
      //     type: "error",
      //     message: "请输入垄断类型",
      //   });
      // } else if (this.formSearch.citys == "" || this.formSearch.citys == null) {
      //   this.$message({
      //     type: "error",
      //     message: "请选择服务城市",
      //   });
          ...
          that.$http
            ...
            .then((res) => {
              if (res.data.success) {
                ...
                this.id = !this.id ? res.data.returnValue1 : this.id;
                that.loading = false;
                this.$router.push({
                  path: "/supplier/specialSupEdit/?id=" + this.id,
                });
                this.activeName = "联系人信息";
                this.lianxirenDataList();
                this.getOutList(res.data.returnValue1);
              } else {
                this.$message({
                  type: "error",
                  message: res.data.message,
    ...
    onOpenCategoryTree() {
      ...
      this.formSearch.supCateNames = this.supCateNames;
    },
    onSelectedCategoryTree(v) {
      console.log(v);
      this.supCategory = v;
      this.formSearch.supCateNames = this.supCateNames;
      this.$refs["saveSupForm"].clearValidate(["supCateNames"]); //取消该项校验
    },
  },
};
</script>
