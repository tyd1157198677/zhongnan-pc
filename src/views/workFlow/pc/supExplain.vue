      ...
      <div style="margin-top: 10px">
        <el-row>
          <el-table empty-text="无符合条件的记录" stripe border element-loading-text="请稍候,数据正在加载中..." :data="tabledata">
            ...
            <el-table-column label="供应商名称" width="300" prop="comFullName" align="left" show-overflow-tooltip> </el-table-column>
            <el-table-column label="澄清说明" width="300" prop="explainContent" align="left"> </el-table-column>
            <el-table-column label="技术澄清类型" prop="explainTechType" align="center"> </el-table-column>
            <el-table-column label="商务澄清类型" prop="explainBusinessType" align="center"> </el-table-column>
            <el-table-column label="相关附件下载" align="center">
              <template slot-scope="scope">
                <el-button type="primary" v-if="scope.row.bFileUrl != null" size="mini" @click="upb(scope.row.businessFileId)"
                  >下载商务澄清函</el-button
                >
                <el-button type="primary" v-if="scope.row.tFileUrl != null" size="mini" @click="upt(scope.row.techFileId)"
                  >下载技术澄清函</el-button
                >
              </template>
            </el-table-column>
...
export default {
  ...
  mounted() {
    ...
    // this.getlist();
    this.getbidModeId();
    this.GetAuditExplainPageShow();
    this.GetSupExplainList();
  ...
  methods: {
    ...
    GetAuditExplainPageShow() {
      this.$http
        .get("/BidExplain/GetAuditExplainPageShow", { params: { masterId: this.masterId, bidModeId: this.bidModeId } })
        .then((res) => {
          this.tabledata = res.data.supList;
    ...
    GetSupExplainList() {
      this.$http.get("/WflowApproveLogsAppServices/GetSupExplainList", { params: { MasterId: this.masterId } }).then((res) => {
        this.tablelist = res.data;
      });
    },
    upt(techFileId) {
      // window.open(`${process.env.VUE_APP_API_URL}/${row.tFileUrl}`, "_blank");
      // return `${process.env.VUE_APP_API_URL}/${row.tFileUrl}`;
      // window.open(process.env.VUE_APP_API_URL + path);
      window.open(process.env.VUE_APP_API_URL + "/api/FileDownLoad/BidExplainDownLoad003?fileId=" + techFileId);
    },
    upb(businessFileId) {
      // window.open(`${process.env.VUE_APP_API_URL}/${row.bFileUrl}`, "_blank");
      // return `${process.env.VUE_APP_API_URL}/${row.bFileUrl}`;
      // window.open(process.env.VUE_APP_API_URL + path);
      window.open(process.env.VUE_APP_API_URL + "/api/FileDownLoad/BidExplainDownLoad003?fileId=" + businessFileId);
    },
    omsubmit() {
      this.$confirm("确认要通过招标过程澄清审批吗?", "提示", {
    ...
    bohui() {
      this.$confirm("确认要驳回招标过程澄清审批吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
