  ...
  <el-main>
    <el-card style="margin-bottom: 60px">
      ...
      <div style="margin-top: 10px">
        <el-row>
          <el-table empty-text="暂无数据" stripe border element-loading-text="请稍候,数据正在加载中..." :data="dataList3.answerFileList">
...
export default {
  ...
  data() {
    return {
      ...
      rulesrizhi: {
        // ideaRemark: [{ required: true, message: "审批意见不能为空", trigger: "change" }],
      },
      rules: {},
    };
  },
  ...
  methods: {
    ...
    del(id) {
      this.$confirm("确定要删除吗?", "提示", {
        ...
        this.$http.post("/LiandoBidSupQs/DeleteAnswerFile003", { id }).then((res) => {
          if (res.data.success) {
    ...
    GetSupQusAuditPageShow() {
      this.$http
        .get("/LiandoBidSupQs/GetSupQusAuditPageShow", { params: { bidModeId: this.bidModeId, masterId: this.masterId } })
        .then((res) => {
    ...
    GetSupQusList() {
      this.$http.get("/WflowApproveLogsAppServices/GetSupQusList", { params: { MasterId: this.masterId } }).then((res) => {
